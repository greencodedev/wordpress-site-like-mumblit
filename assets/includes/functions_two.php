...
function Wo_GetHomeNotification() {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
	$data      = array();
    $user_id      = Wo_Secure($wo['user']['user_id']);
    $query        = mysqli_query($sqlConnect, "SELECT * FROM Wo_Notifications WHERE recipient_id={$user_id} order by id desc");
   while ($fetched_data = mysqli_fetch_assoc($query)) {
        $data[] = $fetched_data;
    }
    return $data;
...
function Wo_JoinGroupSug() {
    global $wo, $sqlConnect;
    $data      = array();
    $user_id   = Wo_Secure($wo['user']['user_id']);
    $query_one = " SELECT `group_id` FROM Wo_Group_Members WHERE `active` = '1' AND `user_id` = {$user_id}";
    $sql = mysqli_query($sqlConnect, $query_one);
    while ($fetched_data = mysqli_fetch_assoc($sql)) {
