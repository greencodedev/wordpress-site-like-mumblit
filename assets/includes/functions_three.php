...
function Wo_SharePost($id = false, $group_id, $page_id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || $id < 1) {
        return false;
    }
    ...
    if ($post) {
        ...
        $query25                  = mysqli_query($sqlConnect, "UPDATE " . T_POSTS . " SET `group_id` = {$group_id} WHERE `id` = {$last}");
        $query25                  = mysqli_query($sqlConnect, "UPDATE " . T_POSTS . " SET `page_id` = {$page_id} WHERE `id` = {$last}");
        $query3                   = mysqli_query($sqlConnect, "UPDATE " . T_POSTS . " SET shares = (shares+1) WHERE `id` = {$last}");        if ($query1 && $query2 && $query3 && Wo_HidePost($id)) {            return true;        }    }    return false;
}
function Wo_CountUserNewViews($user_id) {    global $wo, $sqlConnect;    $data = array();    if (empty($user_id) or !is_numeric($user_id) or $user_id < 1) {        return false;    }    $user_id      = Wo_Secure($user_id);    $query        = mysqli_query($sqlConnect, "SELECT SUM(`newviews`) AS count FROM " . T_POSTS . " WHERE `user_id` = {$user_id}");    $fetched_data = mysqli_fetch_assoc($query);    return $fetched_data['count'];}function Wo_CountUserViews($user_id) {    global $wo, $sqlConnect;    $data = array();    if (empty($user_id) or !is_numeric($user_id) or $user_id < 1) {        return false;    }    $user_id      = Wo_Secure($user_id);    $query        = mysqli_query($sqlConnect, "SELECT SUM(`views`) AS count FROM " . T_POSTS . " WHERE `user_id` = {$user_id}");    $fetched_data = mysqli_fetch_assoc($query);    return $fetched_data['count'];}function Wo_CountUserShares($user_id) {    global $wo, $sqlConnect;    $data = array();    if (empty($user_id) or !is_numeric($user_id) or $user_id < 1) {        return false;    }    $user_id      = Wo_Secure($user_id);    $query        = mysqli_query($sqlConnect, "SELECT SUM(`shares`) AS count FROM " . T_POSTS . " WHERE `user_id` = {$user_id}");    $fetched_data = mysqli_fetch_assoc($query);    return $fetched_data['count'];}function Wo_CountPageViews($page_id = 0) {    global $wo, $sqlConnect;    $data = array();    if (empty($page_id) or !is_numeric($page_id) or $page_id < 1) {        return false;    }    $page_id      = Wo_Secure($page_id);    $query        = mysqli_query($sqlConnect, "SELECT SUM(`views`) AS count FROM " . T_POSTS . " WHERE `page_id` = {$page_id}");    $fetched_data = mysqli_fetch_assoc($query);    return $fetched_data['count'];}function Wo_CountPageNewViews($page_id = 0) {    global $wo, $sqlConnect;    $data = array();    if (empty($page_id) or !is_numeric($page_id) or $page_id < 1) {        return false;    }    $page_id      = Wo_Secure($page_id);    $query        = mysqli_query($sqlConnect, "SELECT SUM(`newviews`) AS count FROM " . T_POSTS . " WHERE `page_id` = {$page_id}");    $fetched_data = mysqli_fetch_assoc($query);    return $fetched_data['count'];}function Wo_CountPageShares($page_id = 0) {    global $wo, $sqlConnect;    $data = array();    if (empty($page_id) or !is_numeric($page_id) or $page_id < 1) {        return false;    }    $page_id      = Wo_Secure($page_id);    $query        = mysqli_query($sqlConnect, "SELECT SUM(`shares`) AS count FROM " . T_POSTS . " WHERE `page_id` = {$page_id}");    $fetched_data = mysqli_fetch_assoc($query);    return $fetched_data['count'];}if (isset($wo['config']['is_ok'])) {    if ($wo['config']['is_ok'] == 0) {        die();    }}
