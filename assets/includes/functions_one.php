...
function Wo_GetGroupsListAPP($fetch_array = array()) {
 ...
 //    return $data;
    ...
    function Wo_RegisterPost($re_data = array('recipient_id' => 0), $from = '') {
        global $wo, $sqlConnect;
    ...
    function Wo_PostData($post_id, $placement = '', $limited = '') {
        ...
        $story['via_type'] = '';
        if (($story['id'] != $fetched_data['id'] && $story['user_id'] != $fetched_data['user_id']) || $story['postType'] == 'share') {
    ...
    function Wo_GetPosts($data = array('filter_by' => 'all', 'after_post_id' => 0, 'page_id' => 0, 'group_id' => 0, 'publisher_id' => 0, 'limit' => 5, 'event_id' => 0, 'ad-id' => 0)) {
        ...
        if (isset($_COOKIE['sense']) && $_COOKIE['sense'] == 1){   
            $subquery_two = " `sense` < 2 ";
        }
        $query_text = "SELECT `id` FROM " . T_POSTS . " WHERE {$subquery_one} AND {$subquery_two} AND `postType` <> 'profile_picture_deleted'";
    ...
    function who_sharesAzhar($post_id, $offset = 0) {
        global $sqlConnect;
        $rows = array();
        $offset *= 15;
        ...
        if ($get_posts->num_rows > 0) {
            while ($row = $get_posts->fetch_assoc()) {
                if ($row['avatar'] != '') {
                    $row['user_picture'] = $row['avatar'];
                } else {
                    $row['user_picture'] = 'upload/photos/d-avatar.jpg';
                }
                if ($row['first_name'] != '') {
