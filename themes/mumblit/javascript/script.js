...
var fixmeTop = $('.rightcol').offset().top;
fixmeTop = fixmeTop+$('.rightcol').height();
var fixemetopwidth=$('.rightcol').width();
$(window).scroll(function() {
    ...
    if (currentScroll >= fixmeTop) {
        $('.rightcol').css({
            position: 'fixed',
            bottom: '0',
            left: $('.posts_load').offset().left+$('.posts_load').width()+15,
      width:fixemetopwidth
        });
    } else {
        $('.rightcol').css({
            position: 'static'
...
$(function () {
  ...
  $('.nav-tabs a').on('shown.bs.tab', function (e) {
    ...
    intervalUpdates = setTimeout(Wo_intervalUpdates, 15000);
...
function Wo_SharePost(post_id, owner_id, self, group_id, page_id) {

  console.log(post_id, owner_id, self, group_id, page_id);
...
function __modal(result) {
    var result_btn = '';
    if (result.btn != '') {
        var result_btn = '<div class="modal-footer">' + result.btn + '</div>';
    }
    var html = '<div class="modal-header">' +
        '<div class="pull-right flip"><button type="button" class="close close_modal" onclick="Wo_CloseModels();"><span>×</span></button></div>' +
...
function _modal(href) {
    if (!$('#modal').is(":visible")) {
        $('#modal').modal('show');
        $('#modal_leader').remove();
        $('body').append('<div class="modal fade in" data-id="modal_leader" id="modal_leader" role="dialog" aria-hidden="false" style="display: block;"><div class="modal-dialog"><div class="modal-content"> <div class="modal-body"><div class="loader pt10 pb10"></div></div> </div></div></div>');
        $('#modal_leader').modal({
            show: true
        });
        if (href) {
            $.ajax({
                ...
                success: function (result) {
                    if (!result) {
                        var modal_html = '<div class="modal-body pt10 pb10">Error try more late</div>';
                    } else {
                        var modal_html = __modal(result);
                    }
...
function ShareIt(self,post_id){
    _modal(Wo_Ajax_Requests_File()+"?f=shareit&only=share&post_id="+post_id)
}
...
function processRegister(data) {
    ...
    if(data.result == 1) {
        ...
        Wo_CloseModels();
        $("#comment_report_box .msg").html('Post has been shared successfully');
        $("#comment_report_box").modal('show');
        setTimeout(function(){
            $("#comment_report_box").modal('hide');
        },3000); 
        ...
        if(data.title != '') {
            $('#modal_leader .modal-title').html(data.title);
            $('#modal_leader .modal-body').html('<div class="mb20 mt20 ml10"><h4>'+data.content+'<h4></div>');
        } else {
...
$('body').on('keyup', '.share_option_name', function () {
    ...
    if (raw_query !== null && raw_query.length >= 2) {
        var query = raw_query;
...
$('body').on('click', '.btn-group a', function (e) {
    ...
    if ($(this).attr('data-id') != '') {
        $(this).closest('.btn-group').parent().find('.share_option').addClass('hidden');
    } else {
        $(this).closest('.btn-group').parent().find('.share_option').removeClass('hidden');
    }
    ...
    $(this).closest('.btn-group').parent().find('.share_option_name').val('').attr({
...
function Mumblit_submit() {
  console.log('In Submit Function');
  user_id = document.querySelector('#userid').value;
  post_id = document.querySelector('#postid').value;
  group_id = document.querySelector('#groupselect').value
  console.log(user_id, post_id, group_id)
  page_id = 0;
  Wo_SharePost(post_id, user_id, this, group_id, page_id);
  }
 
 function showpreview(links,id)
 	{
		
		$.ajax({
    	 type: "GET",
		 url: "https://www.mumblit.com/requests.php",
		 data: "f=update_preview_c&id="+id,
