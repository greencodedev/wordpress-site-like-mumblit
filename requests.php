...
if ($f == 'get_messages') {
    if (Wo_CheckMainSession($hash_id) === true) {
        ...
        $groups_messages = Wo_GetGroupsListAPP(array('limit' => 20));
...
if ($f == 'update_preview_c') {
		$getArrayrow=mysqli_fetch_array(mysqli_query($sqlConnect, "select lclicks from Wo_Posts where post_id='".$_GET['id']."'"));
		mysqli_query($sqlConnect, "UPDATE Wo_Posts SET lclicks = " . ($getArrayrow['lclicks']+1) . " where post_id='".$_GET['id']."'");
	//	echo "UPDATE Wo_Posts SET lclicks = " . ($getArrayrow['lclicks']+1) . " where post_id='".$_GET['id']."'";
		exit();
	}
...
if ($f == 'posts') {
    ...
    if ($s == 'insert_new_post') {
        ...
        if(isset($_POST['sensePost']) && !empty($_POST['sensePost'])) {
            $sense = ($_POST['sensePost'] == 'on') ? 1 : 0;
        }
    ...
    if ($s == 'share-post' && isset($_GET['id']) && is_numeric($_GET['id']) && isset($_GET['usr']) && is_numeric($_GET['usr'])) {
        ...
        $group_id = Wo_Secure($_GET['group_id']);
        $page_id = Wo_Secure($_GET['page_id']);
        if (Wo_SharePost($post_id, $owner, $group_id, $page_id)) {
            $data['status'] = 200;
        }
        header("Content-type: application/json");
        echo json_encode($data);
...
if ($f == 'shareit') {
    ...
    if ($get_post->num_rows == 0) {
        header("Content-type: application/json");
        echo json_encode(array('title' => $wo['lang']['plugin_share_error'], 'content' => $wo['lang']['plugin_share_error_post_no_exists'], 'btn' => ''));
        exit();
    }
    ...
    $html = Wo_LoadPage('story/share/post_sharer');
...
if ($f == 'share_new_post') {
    ...
    $event_id = 0;
    $image_array = array();
    $data = array('title' => $wo['lang']['oops_something_went_wrong'], 'content' => $wo['lang']['please_try_again_later'], 'btn' => 'default', 'result' => 0);
    ...
    $get_post = $sqlConnect->query("SELECT * FROM Wo_Posts WHERE id = '{$post_id}' LIMIT 1");
    if ($get_post->num_rows == 0) {
        $result = array('title' => $wo['lang']['plugin_share_error'], 'content' => $wo['lang']['plugin_share_error_post_no_exists'], 'btn' => '');
        header("Content-type: application/json");
        echo json_encode($result);
    ...
    if ($post['postType'] == "share") {
        $origin = Wo_PostData($post['post_id']);
        $post_id = $origin['post_id'];
        $author_id = $origin['user_id'];
    } else {
        $post_id = $post['post_id'];
    ...
    if (empty($errors)) {
        ...
        $new_post_id = Wo_RegisterPost($post_data, 'shared');

        if ($new_post_id) {
            Wo_CleanCache();
            Wo_UpdateUserDetails($wo['user']['user_id'], true, false, false, true);
            ...
            $data = array('title' => $wo['lang']['oops_something_went_wrong'], 'content' => $wo['lang']['please_try_again_later'], 'btn' => 'Query == ' . $new_post_id . ' -- ', 'result' => 0);
        }
    }
    header("Content-type: application/json");
    echo json_encode($data);
...
if ($f == 'share_who_share') {
    ...
    if (count($users) > 0) {
        ...
        foreach ($users as $_user) {
            include './themes/' . $wo['config']['theme'] . '/layout/story/share_feed.phtml';
        }
...
if ($f == 'autocomplete') {
    ...
    if ($s == 'users') {
        ...
        if (count($skipped_array) > 0) {
            ...
            $sql = "SELECT `user_id`, `username`, `first_name`, `last_name`, `gender`, `avatar` FROM " . T_USERS . " WHERE user_id NOT IN ('{$skipped_list}') AND (username LIKE '%$input%' OR first_name LIKE '$input%' OR last_name LIKE '$input%' )  LIMIT $limit";
        } else {
            $sql = "SELECT `user_id`, `username`, `first_name`, `last_name`, `gender`, `avatar` FROM " . T_USERS . " WHERE (username LIKE '%$input%' OR first_name LIKE '$input%' OR last_name LIKE '$input%')  LIMIT $limit";
        }
        ...
        if ($resource->num_rows > 0) {
            while ($row = $resource->fetch_assoc()) {
                ...
                if ($row['avatar'] != '') {
                    ...
                    $row['photo'] = 'upload/photos/d-avatar.jpg';
                }
                if ($row['first_name'] != '') {
        ...
        if (count($users) > 0) {
            ob_start();
            include './themes/' . $wo['config']['theme'] . '/layout/story/share/ajax_autocomplete.phtml';
            $html = ob_get_contents();
            ob_end_clean();
  ...
  AND (SUBSTRING(username, 1, $len)='$input' OR SUBSTRING(first_name, 1, $len)='$input' OR SUBSTRING(last_name, 1, $len)='$input') LIMIT $limit";
    ...
    if ($s == 'page') {
        ...
        $sql = "SELECT `page_id`, `user_id`, `page_name`, `page_title`, `avatar` FROM Wo_Pages WHERE user_id = '{$wo['user']['user_id']}' AND (page_name LIKE '%$input%' OR page_title LIKE '%$input%') LIMIT $limit";
        $sql = "SELECT `page_id`, `user_id`, `page_name`, `page_title`, `avatar` FROM " . T_PAGES . " 
                   WHERE (page_name LIKE '%$input%' OR page_title LIKE '%$input%') AND (`user_id` = {$wo['user']['user_id']} OR `page_id` IN "
                . "(SELECT `page_id` FROM " . T_PAGE_ADMINS . " WHERE `user_id` = {$wo['user']['user_id']})"
                . ") LIMIT $limit";
        $resource = $sqlConnect->query($sql);
        ...
        if ($resource->num_rows > 0) {
            while ($row = $resource->fetch_assoc()) {
                ...
                if ($row['avatar'] != '') {
                    ...
                    $row['photo'] = 'upload/photos/d-page.jpg';
                }
                if ($row['page_title'] != '') {
                    $row['fullname'] = $row['page_title'];
                } else {
                    $row['fullname'] = $row['page_name'];
                }
                $users[] = $row;
            }
        }
    ...
    if ($s == 'group') {
        ...
        $sql = "SELECT `id`, `user_id`, `group_name`, `group_title`, `avatar` FROM " . T_GROUPS . " WHERE  (group_name LIKE '%$input%' OR group_title LIKE '%$input%') LIMIT $limit";
        $sql = "SELECT `id`, `user_id`, `group_name`, `group_title`, `avatar` FROM " . T_GROUPS . " 
                   WHERE (group_name LIKE '%$input%' OR group_title LIKE '%$input%') AND (`user_id` = {$wo['user']['user_id']} OR `id` IN "
                . "(SELECT `group_id` FROM " . T_GROUP_ADMINS . " WHERE `user_id` = {$wo['user']['user_id']})"
                . ") LIMIT $limit";
        //echo $sql." == <br />";
        $resource = $sqlConnect->query($sql);
        ...
        if ($resource->num_rows > 0) {
            while ($row = $resource->fetch_assoc()) {
                ...
                if ($row['avatar'] != '') {
                    ...
                    $row['photo'] = 'upload/photos/d-group.jpg';
                }
                if ($row['group_title'] != '') {
                    $row['fullname'] = $row['group_title'];
                } else {
                    $row['fullname'] = $row['group_name'];
                }
                $users[] = $row;
            }
        }
    ...
    if ($s == 'event') {
        ...
        $sql = "SELECT `id`, `poster_id`, `name`, `cover` FROM Wo_Events WHERE poster_id = '{$wo['user']['user_id']}' AND (name LIKE '%$input%') LIMIT $limit";
        $sql = "SELECT `id`, `poster_id`, `name`, `cover` FROM " . T_EVENTS . " WHERE poster_id = '{$wo['user']['user_id']}' AND (name LIKE '%$input%') LIMIT $limit";
        $resource = $sqlConnect->query($sql);
        ...
        if ($resource->num_rows > 0) {
            while ($row = $resource->fetch_assoc()) {
                ...
                if ($row['cover'] != '') {
                    ...
                    $row['photo'] = 'upload/photos/d-event.jpg';
                }
                if ($row['name'] != '') {
    ...
    if ($s == 'new_users') {
        ...
        $result = array('result' => 1, 'count' => count($users), 'fof' => $users, 'message' => 'Suggest Friend');
        header("Content-type: application/json");
        echo json_encode($result);
        exit();
    }
